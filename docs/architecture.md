...
## 15. Appuntamenti e sync Google Calendar (nuovo requisito, 2026-07-17)

Pianificazione delle uscite dei tecnici/operai dal cliente, visibile in un calendario dentro
l'app e, opzionalmente, sul calendario personale del tecnico (Google Calendar — e di riflesso su
Calendario iOS, che supporta nativamente gli account Google senza bisogno di un'integrazione
Apple separata).
...
```
id, tenant_id,
customer_id (nullable), technician_id (FK users, stesso pattern di ServiceReport.technician_id),
comodato_macchina_id (nullable), deadline_id (nullable — collega a una scadenza es. manutenzione),
service_report_id (nullable — valorizzato alla chiusura),
title, starts_at, ends_at,
status (enum: pianificato, confermato, in_corso, completato, annullato),
notes, google_event_id, google_synced_at, created_at, updated_at
```

`status` copre gli stati della visita (`in_corso` = check-in del tecnico). Eventuale
geolocalizzazione al check-in è rimandata a una fase successiva (non bloccante per la v1).

**`google_calendar_accounts`** — un collegamento OAuth per utente (opt-in, non a livello di
tenant), con un **calendario secondario dedicato** ("CRM - Lavoro") creato via API alla prima
connessione: la sync bidirezionale non tocca mai il calendario personale/primario dell'utente.

```
id, tenant_id, user_id (unique),
google_account_email, access_token (encrypted), refresh_token (encrypted), token_expires_at,
calendar_id, sync_token, connected_at, created_at, updated_at
```

### 15.2 Sync

Push (CRM → Google) via Observer + job in coda ad ogni save/delete di `Appointment`. Pull
(Google → CRM) via comando schedulato ogni 15 minuti con `syncToken` incrementale — niente
webhook push di Google per la v1 (richiederebbero un endpoint HTTPS pubblico verificato),
coerente con l'approccio "non bloccante per la prima versione" già usato per il tableau
assistenze (§13.2).

### 15.3 Calendario in-app

Pagina Filament con `saade/filament-fullcalendar`, filtrabile per tecnico, drag & drop per
riprogrammare. Per gli appuntamenti assorbe il ruolo del tableau descritto in §13.2 (mai
implementato); le `Deadline` senza appuntamento collegato restano nella lista semplice li
prevista.
