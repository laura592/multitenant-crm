@php
    ...
    // Fix locale: col solo tenant "Alex" attivo (nessun altro partner onboardato
    // ancora, nessuna tenantProfile/tenantMenuItems/registration configurata),
    // il pannello del dropdown Filament resta completamente vuoto - il bottone
    // pero' mostrava comunque la freccina "v", segnale visivo di un menu che in
    // realta' non apre nulla (segnalato dall'utente). Quando non c'e' nessun
    // contenuto reale, si mostra la stessa riga come etichetta statica, senza
    // freccina ne' hover/cursor da elemento cliccabile. Il dropdown Filament
    // torna automaticamente com'era appena si aggiunge un secondo tenant,
    // una tenantProfile o menu item custom (una qualsiasi di queste condizioni
    // diventa vera).
    $hasTenantMenuContent = $hasProfileItem || $hasBillingItem || count($items) || $canSwitchTenants || $hasRegistrationItem;
@endphp

{{ \Filament\Support\Facades\FilamentView::renderHook(\Filament\View\PanelsRenderHook::TENANT_MENU_BEFORE) }}

@if ($hasTenantMenuContent)
    <x-filament::dropdown
